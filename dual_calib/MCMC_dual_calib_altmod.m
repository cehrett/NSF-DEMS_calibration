function results = MCMC_dual_calib_altmod(settings)
...
des_cov_mat                   = settings.des_cov_mat;
...
D = [ sim_y ; obs_y ; des_y ] ;
% T is the vector of the simulator outputs and target observations.
T = [ sim_y ; des_y ] ;
...
%% Define functions for updating observation discrepancy (in modular case) 
add_nug = @(X) X+eye(size(X))*nugsize(X);
...
Sigma_R = [... % The covariance of R
    ...
    Sigma_emulator_simobs' ...
        ...
        additional_discrep_cov(theta1) ... % By default adc() is zero
    ];
...
for ii = 2:M
    ...
    Sigma_R_s = [...
        ...
        Sigma_emulator_simobs_s' ...
            Sigma_emulator_obsobs_s + Sigma_obs_dscr_obsobs + ...
                obs_cov_mat + additional_discrep_cov(theta1_s)
    ];
    Sigma_D_s = [...
    ...
    if modular % if modular, calculate likelihood without observations
        obs_discrep_vals = obs_y - ...
            mean_sim(obs_x,repmat(theta1,size(obs_x,1),1),obs_t2);
        ...
        % Now we can get the log factors of the likelihood for the new draw
        log_cond_dens_T_s = logmvnpdf(T',mu_T_s',Sigma_T_s);
        
        % Get the likelihood for the new draw:
        log_lik_theta2_s = log_cond_dens_T_s + log_theta2_prior_s; 
        
        
        % Now we have to calculate the likelihood of the previous t2 draw.
        % We can't just use it saved from last time, because other vars may
        % have changed.
        cov_obs_disc_upd = updated_cov(obs_x,obs_t2,...
            des_x, repmat(theta2,size(des_x,1),1),...
            obs_rho,obs_lambda); % cov_obs_disc_upd is NOT updated with 
                                 % obs_rho, obs_lambda.
        mean_obs_disc_upd = updated_mean(obs_discrep_vals,obs_x,obs_t2,...
            des_x, repmat(theta2,size(des_x,1),1),...
            obs_rho,obs_lambda);
        ...
        Sigma_T = Sigma_T + eye(size(Sigma_T)) * nugsize(Sigma_T) ;
        % Now we can get the log factors of the likelihood for the old draw
        log_cond_dens_T = logmvnpdf(T',mu_T',Sigma_T);
        
        % And we get the likelihood for the old draw
        log_lik_theta2 = log_cond_dens_T + log_theta2_prior;
    else
