...
settings = MCMC_dual_calib_settings(sim_x,sim_t1,sim_t2,sim_y,...
    ...
    des_x,des_y,...
    'min_x',.5,'range_x',.5,...
    'min_t1',1.5,'range_t1',3,...
    'min_t2',0,'range_t2',5,...
    'mean_y',mean_y,'std_y',std_y,...
    'burn_in',.5,...
    'EmulatorCovHypers',[.5 .5 .5 Inf],...
    'obs_discrep',false,...
    'emulator_use',false);
...
obs_y_noiseless = dual_calib_example_fn((obs_x-xmin)/xrange,xmin,xrange,...
    ...
    (obs_t2-t2min)/t2range,t2min,t2range,...
    0,1,...
    discrep,...
    true);
...
int_fn =@(x,t)...
    dual_calib_example_fn(x,Inf,1,...
    theta1*ones(size(x)),0,1,...
    t,0,1,...
    0,1,...
    discrep,...
    false)-...
...
Y = reshape(dual_calib_example_fn(X(:),xmin,xrange,...
    ...
    0,1,...
    0,...
    true),length(x),length(t1),length(t2));
Yd= reshape(dual_calib_example_fn(X(:),xmin,xrange,...
    T1(:),t1min,t1range,...
    T2(:),t2min,t2range,...
    0,1,...
    discrep,...
...
%             [ones(size(x,1),1) x t] * obs_discrep_coeffs ; 
        ...
        % Emulator mean
        mean_sim = @(abc) model_fn(abc(:,1),abc(:,2),abc(:,3)) ; 
...
%             'obs_var_est',true);
        ...
        KOH_mean_sim = @(x_t2_t1) model_fn(x_t2_t1(:,1),...
            x_t2_t1(:,3),x_t2_t1(:,2));
        ...
        % Define objective function
        CTO_mean_sim = @(x_t2) model_fn(x_t2(:,1), mean(theta1_samps), ...
            x_t2(:,2)) ;
...
%     '2019-10-04_DCTO_vs_KOHCTO_results_temp'];
    locstr = [dpath,'dual_calib\dual_calib_stored_data\',...
    '2020-06-29_DCTO_vs_KOHCTO_results_temp'];
%     save(locstr,'results');
...
%         dcto_results.obs_var_hat(ii) = mean(res.obs_var(burn_in:end));
        ...
        % Set simulator mean for KOH
        KOH_mean_sim = @(x_t2_t1) model_fn(x_t2_t1(:,1),x_t2_t1(:,3),x_t2_t1(:,2));
...
%         khtc_results.obs_var_hat(ii) = mean(res.obs_var(burn_in:end));
        ...
        % Define objective function
        CTO_mean_sim = @(x_t2) model_fn(x_t2(:,1), mean(theta1_samps), x_t2(:,2)) ;
