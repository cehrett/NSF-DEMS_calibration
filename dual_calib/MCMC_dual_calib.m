...
mu_all = [mean_sim(sim_x,sim_t1,sim_t2) ; ...
...
log_cond_dens_D = logmvnpdf(D',mu_all',Sigma);
...
for ii = 2:M
    ...
    % Add nugget for computational tractability
    Sigma_s = Sigma_s + eye(size(Sigma_s)) * nugsize(Sigma_s) ;
    ...
    Sigma_s = Sigma_s + eye(size(Sigma_s)) * nugsize(Sigma_s) ;
    % Now we can get the log factors of the likelihood for the new draw
    log_cond_dens_D_s = logmvnpdf(D',mu_all',Sigma_s);
