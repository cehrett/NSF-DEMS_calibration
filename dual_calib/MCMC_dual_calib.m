...
for ii = 2:M
    ...
    if log(rand) < log_alpha
        ...
        if modular % Need to update log_cond_dens_D
            log_cond_dens_D_s = logmvnpdf(D',mu_D_s',Sigma_D_s);
        end
    ...
    log_obs_rho_prior_s = log_obs_rho_prior_fn(obs_rho_s);
    ...
    if mod(ii,upd) == 0 && ii <= burn_in
        ...
        if verbose
            fprintf(...
                'obs_lambda proposal variance set to %g of previous\n',...
                mult_mult);
        end
        ...
        if verbose
            fprintf(...
                'des_lambda proposal variance set to %g of previous\n',...
                mult_mult);
        end
