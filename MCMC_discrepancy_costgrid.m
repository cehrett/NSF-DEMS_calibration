...
%% Define obs_x_nc, which is the control settings for non-cost des obs.
% This is needed since this version of the code is for the use of a cost
% grid, where costs are treated as known. So there's no discrepancy on
% costs.
obs_x_nc = obs_x(1:size(obs_x,1)*(num_out-1)/num_out,:);
...
Sigma_delta=gp_cov(omega_delta,obs_x_nc,obs_x_nc,0,0,0,lambda_delta,false);
Sigma_delta = padarray(Sigma_delta,[num_obs num_obs],'post');
...
for ii = 1:M
    ...
    Sigma_delta_s = ...
        gp_cov(omega_delta_s,obs_x_nc,obs_x_nc,0,0,0,lambda_delta,false);
    Sigma_delta_s = padarray(Sigma_delta_s,[num_obs num_obs],'post');
    % Combine these to get new Sigma_z
    ...
    % Set new discrep covariance matrix:
    Sigma_delta_s = ...
        gp_cov(omega_delta,obs_x_nc,obs_x_nc,0,0,0,lambda_delta_s,false);
    Sigma_delta_s = padarray(Sigma_delta_s,[num_obs num_obs],'post');
    % Combine these to get new Sigma_z
    Sigma_z_s=[Sigma_eta_yy+Sigma_y+Sigma_delta_s Sigma_eta_yx ; ...
               Sigma_eta_xy                       Sigma_eta_xx ] ;
