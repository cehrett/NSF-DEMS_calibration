...
function res = grid_optimize_rho(dat, omega1, omega2, rho1, rho2)
...
for ii=1:length(rho1)
    for jj=1:length(rho2)
        ...
        res.rho1(ii,jj)        = rho(1);
        res.rho2(ii,jj)        = rho(2);
        res.rho_mspe(ii,jj)   = MSPE;
        count = count + 1; pp = 100*count / (length(rho1)*length(rho2));
